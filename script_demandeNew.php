...
<?php
    ...
    /* On va enregistrer la date et l'heure de création et publication de la nouvelle demande. 
    ...
    /* Nous récupérons les informations passées dans le fichier "demandeNew.php" dans la balise <form> et l'attribut action="script_demandeNew.php".  
    ...
    if(isset($_POST['titre']) && isset($_POST['description']) && isset($_POST['budget']) && isset($_POST['etat']))
    {
        if (!empty($_POST['titre'] && $_POST['description'] && $_POST['budget'] && $_POST['etat']))
        {
            ...
            /* Vérification si le nom de la demande déjà existe dans base de donnée, car le client ne peut pas créer deux 
            demandes qui portent le même nom.   */
            
            // Connexion à la base de données:     
            require ("connection_bdd.php");

            // Construction de la requête SELECT
            $requete = $db->prepare('SELECT demande_titre FROM demande WHERE user_email=:user_email');  
            ...
            $requete->execute(array(':user_email' => $_SESSION['email']));
            ...
            if ($nbLigne >= 1)
            {
                ...
                {                                               // Ensuite avec la boucle "while" on choisit 2eme, 3eme, etc.. lignes de la colonne equipe_nom et on les mets dans l'objet $row 
                    if ($row->demande_titre==$demande_titre)
                    {
                        echo'<div class="container-fluid alert alert-warning mt-5" role="alert">
                                <center> 
                                    <h4> Cette demande déjà existe. Veuillez choisir un autre nom pour votre nouvelle demande ! </h4> 
                                </center>
                            </div>'; 
            ...
            { 
                ...
                Ici on définit la maximale taille autorisée comme 5 000 000 octets (soit 5Mo) :     */
                $taille_maxi = 5000000;
                ...
                /* Maintenant on va vérifier le type du fichier téléchargé par l'utilisateur. 
                Pour cela d'abord on met les types autorisés dans un tableau (array) nommé $extensions_valide :     */
                ...
                {
                    // Le type du fichier n'est pas autorisé, donc message d'ERREUR
                    echo '<div class="container-fluid alert alert-warning mt-5" role="alert">
                            <center> 
            ...
            $requete = $db->prepare("INSERT INTO demande (demande_proprietaire, demande_societe, demande_titre, demande_description, 
            demande_budget, demande_file_name, demande_creation, demande_publication, demande_equipe, demande_etat, demande_notification, 
            equipe_id, user_id, user_email) VALUES((SELECT CONCAT(user_nom, ' ', user_prenom) AS demande_proprietaire FROM users WHERE user_email=:user_email), 
            (SELECT user_societe FROM users WHERE user_email=:user_email), :demande_titre, :demande_description, :demande_budget, 
            :demande_file_name, :demande_creation, :demande_publication, :demande_equipe, :demande_etat, :demande_notification, 
            (SELECT equipe_id FROM equipe WHERE user_email=:user_email AND equipe_nom=:demande_equipe), 
            (SELECT user_id FROM users WHERE user_email=:user_email), :user_email)");
            ...
            // Maintenant, on va déplacer et renommer le fichier. Pour renommer le fichier on a besoin d'avoir numéro id de la demande:
            $requete = $db->prepare("SELECT demande_id FROM demande WHERE demande_titre=:demande_titre AND user_email=:user_email");
            ...
            /* Par défaut, le fichier téléchargé par utilisateur est stocké dans le répertoire tmp (temporary) de notre serveur 
            Wamp dans C:/wamp/tmp. Mais ce fichier devra se trouver dans un répertoire de notre projet, il faut donc le déplacer.
            Donc, via la méthode "move_uploded_file()" on va déplacer notre fichier depuis répertoire C:/wamp/tmp 
            vers le répertoire "fichiers" de notre projet. Le nouveau nom du fichier sera par exemple "demande_5 Isothermique".   */  
            move_uploaded_file($_FILES["clientFile"]["tmp_name"], "fichiers/demande_".$row->demande_id." ".$nom_fichier);   
            
            echo '<div class="container-fluid alert alert-success mt-5" role="alert">
                    <center> 
                        <h4> Votre demande a été crée avec succès! </h4> 
                    </center>
                </div>'; 
            ...
            /* Association des valeurs aux marqueurs et execution de requête:
            L'écriture raccourcie: ici la méthode bindValue sera appellée "automatiquement"     */     
