<?php
    // Pour utiliser nos variables "$_SESSION" il faut ajouter le fonction session_start() au début de la page:
    session_start();  
    ...
    if(isset($_POST['titre']) && isset($_POST['desc']) && isset($_POST['budget']) && isset($_POST['etat']))
    {
        if (!empty($_POST['titre'] && $_POST['desc'] && $_POST['budget'] && $_POST['etat']))
        {
            ...
            $demande_description = htmlspecialchars($_POST['desc']);
            ...
            $requete = $db->prepare("INSERT INTO demande (demande_titre, demande_description, demande_budget, demande_creation, 
            demande_publication, demande_etat, user_id, user_email) VALUES (:demande_titre, :demande_description, :demande_budget, 
            :demande_creation, :demande_publication, :demande_etat, (SELECT user_id FROM client WHERE user_email=:mail), :user_email)");
    ...
    { 
        // D'abord on vérifie la taille du fichier:
        $taille_maxi = 5000000;
        $taille_fichier = filesize($_FILES['fichier']['tmp_name']);  // La fonction filesize () renvoie la taille d'un fichier.
        ...
        // Ensuite on vérifie le type du fichier. Pour cela on met les types autorisés dans un tableau nommé $extensions_valide :  
        $extensions_valide = array(".pdf", ".doc", ".docx", ".txt");
        
        $extension_fichier = strtolower(strrchr($_FILES['fichier']['name'], '.'));   // fonction strrchr() nous renvoie le string à partir de "point" qui correspond à l'extension du fichier
